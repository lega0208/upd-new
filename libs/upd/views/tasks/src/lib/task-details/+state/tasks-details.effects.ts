...
export class TasksDetailsEffects {
  init$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([, taskId, { dateRange, comparisonDateRange }, taskDetailsData]) => {
        if (!taskId) {
        ...
        if (taskIsLoaded && dateRangeIsLoaded && comparisonDateRangeIsLoaded) {
          // if everything is already loaded in the state, don't update it
          return of(
            loadTasksDetailsSuccess({ data: null })
          );
        }

        return this.api
          .getTasksDetailsData({ id: taskId, dateRange, ...{ comparisonDateRange } })
          .pipe(
            map((data) =>
              loadTasksDetailsSuccess({ data })
            ),
            catchError(() => EMPTY)
          );
      })

...
import { withTimeout } from '../utils';
  ...
  export class SearchAnalyticsClient {
    ...
    async getOverallMetrics(dateRange: { start: string, end: string }) {
      ...
      .addDimensions( 'date' )
      .setFilter([
        { dimension: 'page', operator: 'includingRegex', expression: '/en/revenue-agency|/fr/agence-revenu|/en/services/taxes|/fr/services/impots' }
      ])
      .setDataState( 'final')
      .build();
      ...
      const results = await withTimeout<any>(() => this.client.searchanalytics.query({

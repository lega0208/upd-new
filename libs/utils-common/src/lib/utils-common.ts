...
type Metric = 'visits' | 'calls' | 'dyf_no' | 'survey';
const METRIC_KEYS: Metric[] = ['visits', 'calls', 'dyf_no', 'survey'];

interface DistributionStats { min: number; max: number; p5: number; p95: number; }
type StatsByMetric = { visits: DistributionStats; calls: DistributionStats; dyf_no: DistributionStats; survey: DistributionStats };
type MetricWeights = { visits: number; calls: number; dyf_no: number; survey: number };

export const METRIC_WEIGHTS: MetricWeights = { visits: 10, calls: 50, dyf_no: 30, survey: 10 };

// Inclusive percentile on a pre-sorted ascending array
const calculatePercentile = (sortedAsc: number[], percentile: number): number => {
...
const computeDistributionStats = (arr: number[]): DistributionStats => {
  ...
  for (let i = 1; i < data.length; i++) { const v = data[i]!; if (v < min) min = v; if (v > max) max = v; }
...
const normalizeWithinPercentileRange = (rawValue: unknown, p5th: unknown, p95th: unknown): number | undefined => {
  const value = Number(rawValue), low = Number(p5th), high = Number(p95th);
  if (!Number.isFinite(value) || !Number.isFinite(low) || !Number.isFinite(high)) return undefined;
  const span = high - low; if (!(span > 0)) return undefined;
  const clamped = Math.max(Math.min(value, high), low);
  return (clamped - low) / span;
};

const tailBonusAboveP95 = (rawValue: unknown, p95th: unknown, rawMax: unknown, tailCap: number): number => {
  const value = Number(rawValue), p95 = Number(p95th), maxVal = Number(rawMax);
  if (!Number.isFinite(value) || !Number.isFinite(p95) || !Number.isFinite(maxVal)) return 0;
...
export function computeMetricWeightedScore(rawValue: unknown, p5th: number, p95th: number, maxVal: number, weight: number): number | undefined {
...
export async function getGlobalMetricStats(tasks: TasksHomeAggregatedData[]): Promise<StatsByMetric> {
  const metrics = { visits: [], calls: [], dyf_no: [], survey: [] } as Record<Metric, number[]>;
  ...
  return Object.fromEntries(Object.entries(metrics).map(([k, v]) => [k, computeDistributionStats(v)])) as StatsByMetric;
}

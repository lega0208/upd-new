    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        return (
            df.filter(
                ...
                pl.col("aa_searchterms_fr").list.len() != 0,
            )
            .rename({"_id": "_doc_id"})
            .explode("aa_searchterms_fr")
            .with_columns(pl.col("aa_searchterms_fr").struct.unnest())
            .drop("aa_searchterms_fr")
            .with_columns(
        ...
        return (
            df.select(
                [
                    ...
                    ).alias("aa_searchterms_fr"),
                ]
            )
            .group_by("_id")
            .agg(
                pl.col("aa_searchterms_fr").implode(),
            )
        )
